/*
 *  iaf_neuron_dif_alpha.h
 *
 *
 */

#ifndef IAF_NEURON_DIF_ALPHA_H
#define IAF_NEURON_DIF_ALPHA_H
...
namespace nest
{
  ...
  /* BeginDocumentation
Name: iaf_neuron_dif_alpha - Leaky integrate-and-fire neuron model.

Description:

  iaf_neuron_dif_alpha is an implementation of a leaky integrate-and-fire model
...
*/
   ...
   */
  class iaf_neuron_dif_alpha: public Archiving_Node
  {
    
  public:
    
    iaf_neuron_dif_alpha();
    iaf_neuron_dif_alpha(const iaf_neuron_dif_alpha&);
    ~iaf_neuron_dif_alpha();
  ...
  private:
    ...
    friend class RecordablesMap<iaf_neuron_dif_alpha>;
    friend class UniversalDataLogger<iaf_neuron_dif_alpha>;
     ...
     */
     struct Buffers_ {
      Buffers_(iaf_neuron_dif_alpha &);
      Buffers_(const Buffers_ &, iaf_neuron_dif_alpha &);
     ...
     /** buffers and summs up incoming spikes/currents */
      ...
      UniversalDataLogger<iaf_neuron_dif_alpha> logger_;
    };
   ...
   static RecordablesMap<iaf_neuron_dif_alpha> recordablesMap_;
...
inline
port iaf_neuron_dif_alpha::check_connection(Connection& c, port receptor_type)
{
  SpikeEvent e;
  e.set_sender(*this);
  c.check_event(e);
...
inline
port iaf_neuron_dif_alpha::connect_sender(SpikeEvent&, port receptor_type)
{
  if (receptor_type != 0)
...
inline
port iaf_neuron_dif_alpha::connect_sender(CurrentEvent&, port receptor_type)
{
  if (receptor_type != 0)
    throw UnknownReceptorType(receptor_type, get_name());
  return 0;
...
inline
port iaf_neuron_dif_alpha::connect_sender(DataLoggingRequest &dlr, 
		      port receptor_type)
{
  if (receptor_type != 0)
    throw UnknownReceptorType(receptor_type, get_name());
...
inline
void iaf_neuron_dif_alpha::get_status(DictionaryDatum &d) const
{
  P_.get(d);
  S_.get(d, P_);
...
inline
void iaf_neuron_dif_alpha::get_status_user(DictionaryDatum &d) const
{
  behav_learning_loc->get(d);//KAK
}

inline
void iaf_neuron_dif_alpha::set_status(const DictionaryDatum &d)
{
...
inline
double iaf_neuron_dif_alpha::alpha_function(double t, double tau_a)
{
	return (t/tau_a)*exp(-t/tau_a)*heaviside(t);
}

inline 
double iaf_neuron_dif_alpha::gauss(double t,double mu,double sigma)
{
...
/*
inline
double iaf_neuron_dif_alpha::get_sum_I_learn(double_t t)
{
...
#endif /* #ifndef iaf_neuron_dif_alpha_H */
