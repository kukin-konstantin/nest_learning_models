...
#ifndef STDP_POISSON_H
#define STDP_POISSON_H
...
namespace nest
{
  class STDPPoisson : public ConnectionHetWD
  {
...
inline 
void STDPPoisson::check_connection(Node & s, Node & r, rport receptor_type, double_t t_lastspike)
{
 ...
 */
inline
void STDPPoisson::send(Event& e, double_t t_lastspike, const CommonSynapseProperties &)
{
  ...
  {
    minus_dt = t_lastspike - (post_first_spike->t_ + dendritic_delay);
    if (minus_dt != 0)
      weight_ = facilitate_(weight_, std::exp(minus_dt / tau_plus_));
  }*/
