...
nest::RecordablesMap<nest::iaf_psc_exp_mode> nest::iaf_psc_exp_mode::recordablesMap_;
...
namespace nest
{
  ...
  template <>
  void RecordablesMap<iaf_psc_exp_mode>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &iaf_psc_exp_mode::get_V_m_);
  }
}
...
nest::iaf_psc_exp_mode::Parameters_::Parameters_()
  : Tau_     (  10.0       ),  // in ms
    C_       ( 250.0       ),  // in pF
    t_ref_   (   2.0       ),  // in ms
    U0_      ( -70.0       ),  // in mV
...
nest::iaf_psc_exp_mode::State_::State_()
  : i_0_      ( 0.0 ),
    i_syn_ex_ ( 0.0 ),
    i_syn_in_ ( 0.0 ),
    V_m_      ( 0.0 ),
...
void nest::iaf_psc_exp_mode::Parameters_::get(DictionaryDatum &d) const
{
  ...
  def<double>(d,names::I_syn,I_syn_);
}

void nest::iaf_psc_exp_mode::Parameters_::set(const DictionaryDatum &d)
{
...
void nest::iaf_psc_exp_mode::State_::get(DictionaryDatum &d, const Parameters_ &p) const
{
  def<double>(d, names::V_m, V_m_ + p.U0_); // Membrane potential
}

void nest::iaf_psc_exp_mode::State_::set(const DictionaryDatum &d, const Parameters_ &p)
{
  if ( updateValue<double>(d, names::V_m, V_m_) )
    V_m_ -= p.U0_;
}

nest::iaf_psc_exp_mode::Buffers_::Buffers_(iaf_psc_exp_mode &n)
  : logger_(n)
{}

nest::iaf_psc_exp_mode::Buffers_::Buffers_(const Buffers_ &, iaf_psc_exp_mode &n)
  : logger_(n)
{}
...
nest::iaf_psc_exp_mode::iaf_psc_exp_mode()
  : Archiving_Node(), 
    P_(), 
    S_(),
    B_(*this)
...
nest::iaf_psc_exp_mode::iaf_psc_exp_mode(const iaf_psc_exp_mode &n)
  : Archiving_Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
...
void nest::iaf_psc_exp_mode::init_state_(const Node &proto)
{
  const iaf_psc_exp_mode &pr = downcast<iaf_psc_exp_mode>(proto);
  S_ = pr.S_;
}

void nest::iaf_psc_exp_mode::init_buffers_()
{
...
void nest::iaf_psc_exp_mode::calibrate()
{
  ...
  // a double_t in ms. The grid based iaf_psc_exp_mode can only handle refractory
...
void nest::iaf_psc_exp_mode::update(const Time &origin, const long_t from, const long_t to)
{
  ...
  {	
    ...
    P_.I_syn_=S_.i_syn_ex_+ S_.i_syn_in_;
...
void nest::iaf_psc_exp_mode::handle(SpikeEvent &e)
{
...
void nest::iaf_psc_exp_mode::handle(CurrentEvent &e)
{
  assert ( e.get_delay() > 0 );
...
void nest::iaf_psc_exp_mode::handle(DataLoggingRequest &e)
{
  B_.logger_.handle(e);
}
