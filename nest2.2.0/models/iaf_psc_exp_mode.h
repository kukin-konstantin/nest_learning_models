...
#ifndef IAF_PSC_EXP_MODE_H
#define IAF_PSC_EXP_MODE_H
...
namespace nest
{
  ...
  /* BeginDocumentation
     Name: iaf_psc_exp_mode - Leaky integrate-and-fire neuron model with exponential PSCs.

     Description:
     iaf_psc_exp_modep is an implementation of a leaky integrate-and-fire model
     ...
     optimization levels. A future version of iaf_psc_exp_mode will probably
  ...
  */
  ...
  public:        
    ...
    iaf_psc_exp_mode();
    iaf_psc_exp_mode(const iaf_psc_exp_mode&);
  ...
  private:
    ...
    friend class RecordablesMap<iaf_psc_exp_mode>;
    friend class UniversalDataLogger<iaf_psc_exp_mode>;
    ...
    struct Buffers_
    {
      Buffers_(iaf_psc_exp_mode &);
      Buffers_(const Buffers_ &, iaf_psc_exp_mode &);
      ...
      UniversalDataLogger<iaf_psc_exp_mode> logger_;
    };
    ...
    /**
     * @defgroup iaf_psc_exp_mode_data
    ...
    static RecordablesMap<iaf_psc_exp_mode> recordablesMap_;
  };

  inline
  port iaf_psc_exp_mode::check_connection(Connection &c, port receptor_type)
  {
    SpikeEvent e;
    e.set_sender(*this);
    c.check_event(e);
  ...
  inline
  port iaf_psc_exp_mode::connect_sender(SpikeEvent&, port receptor_type)
  {
    if (receptor_type != 0)
  ...
  inline
  port iaf_psc_exp_mode::connect_sender(CurrentEvent &, port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
    return 0;
  ...
  inline
  port iaf_psc_exp_mode::connect_sender(DataLoggingRequest &dlr, 
				   port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline
  void iaf_psc_exp_mode::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d, P_);
  ...
  inline
  void iaf_psc_exp_mode::set_status(const DictionaryDatum &d)
  {
