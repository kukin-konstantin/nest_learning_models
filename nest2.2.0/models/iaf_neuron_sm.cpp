...
int nest::iaf_neuron_sm::id_gl=-1; //nebodimo dlya togochtobi numeracia neuronov nacinalas s edinici
nest::RecordablesMap<nest::iaf_neuron_sm> nest::iaf_neuron_sm::recordablesMap_;
...
namespace nest
{
  ...
  template <>
  void RecordablesMap<iaf_neuron_sm>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &iaf_neuron_sm::get_V_m_);
  }
}
...
nest::iaf_neuron_sm::Parameters_::Parameters_()
  : C_      (200.0    ),  // pF
    Tau_    ( 10.0    ),  // ms
    TauR_   (  2.0    ),  // ms
    E_L_     (-70.0    ),  // mV
...
nest::iaf_neuron_sm::State_::State_()
  : 
  v_(-70.0),
    I_(0.0),  
    r_ (0)
...
nest::iaf_neuron_sm::~iaf_neuron_sm()
{
  delete behav_loc;
  delete behav_learning_loc;
  std::cout<<"iaf_neuron_sm destruct counter="<<iaf_neuron_sm::id_gl<<"\n";
  iaf_neuron_sm::id_gl--;
  ...
  //delete behav;
   ...
   for (int i=0;i!=v_learn_set_input.size();i++)
   {
     for (int j=0;j!=v_learn_set_input[i].size();j++)
     {
       for (int k=0;k!=v_learn_set_input[i][j].size();k++)
       {
	  std::cout<<v_learn_set_input[i][j][k]<<"\t";
       }
        std::cout<<"\n";
     }
     std::cout<<"\n\n";
...
void nest::iaf_neuron_sm::Parameters_::get(DictionaryDatum &d) const
{
...
void nest::iaf_neuron_sm::Parameters_::set(const DictionaryDatum& d)
{
...
void nest::iaf_neuron_sm::State_::get(DictionaryDatum &d, const Parameters_& p) const
{
  def<double>(d, names::V_m, v_); // Membrane potential
}

void nest::iaf_neuron_sm::State_::set(const DictionaryDatum& d, const Parameters_& p)
{
  updateValue<double>(d, names::V_m, v_);
}


nest::iaf_neuron_sm::Buffers_::Buffers_(iaf_neuron_sm &n)
  : logger_(n)
{}

nest::iaf_neuron_sm::Buffers_::Buffers_(const Buffers_ &, iaf_neuron_sm &n)
  : logger_(n)
{}
    ...
    B_(*this)
{
  ...
  std::cout<<"non-copy iaf_neuron_sm::id_gl "<<iaf_neuron_sm::id_gl<<"\n"; 
...
nest::iaf_neuron_sm::iaf_neuron_sm(const iaf_neuron_sm& n)
  : Archiving_Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
{
  ...
  std::stringstream r;
  iaf_neuron_sm::id_gl++;
  std::cout<<"copy iaf_neuron_sm::id_gl "<<iaf_neuron_sm::id_gl<<"\n";
  r << iaf_neuron_sm::id_gl;
  ...
  behav_learning_loc->read_learn_set_input_from_files(s_directory_learn_set_input,iaf_neuron_sm::id_gl); //ano
  /*learn function*/
}

void nest::iaf_neuron_sm::set_learning_variables(DictionaryDatum &d,double t,std::vector<double > &currents)//prepare before learning
{
...
void nest::iaf_neuron_sm::init_state_(const Node& proto)
{
  const iaf_neuron_sm& pr = downcast<iaf_neuron_sm>(proto);
  S_ = pr.S_;
}



void nest::iaf_neuron_sm::init_buffers_()
{
...
void nest::iaf_neuron_sm::calibrate()
{
  B_.logger_.init();
...
double nest::iaf_neuron_sm::normalization(double_t v)
{
...
bool nest::iaf_neuron_sm::emission_procedure(double_t v)
{
  double Th=1.0;
  double k=0.1;
...
void nest::iaf_neuron_sm::update(Time const & origin, const long_t from, const long_t to)
{
  //bool ok=true;
  ...
  if (trig_seal_)
  {
    ...
    behav_learning_loc->fill_gradient_zero();
    clear_spike_history();
  }
  if (trig_learn_oper_)
  {
	...
	S_.I_ =S_.I_+get_sum_I_learn(t+h*lag); //two time calclulate
...
void nest::iaf_neuron_sm::handle(SpikeEvent & e)
{
...
void nest::iaf_neuron_sm::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::iaf_neuron_sm::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}
