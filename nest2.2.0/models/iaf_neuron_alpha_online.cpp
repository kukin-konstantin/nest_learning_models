/*
 *  iaf_neuron_alpha_online.cpp
 *
 *
 */

#include "exceptions.h"
#include "iaf_neuron_alpha_online.h"
#include "network.h"
#include "dict.h"
...
int nest::iaf_neuron_alpha_online::id_gl=-1; //nebodimo dlya togochtobi numeracia neuronov nacinalas s edinici
nest::RecordablesMap<nest::iaf_neuron_alpha_online> nest::iaf_neuron_alpha_online::recordablesMap_;
...
namespace nest
{
  ...
  template <>
  void RecordablesMap<iaf_neuron_alpha_online>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &iaf_neuron_alpha_online::get_V_m_);
  }
}
...
nest::iaf_neuron_alpha_online::Parameters_::Parameters_()
  : C_      (200.0    ),  // pF
    Tau_    ( 10.0    ),  // ms
    TauR_   (  2.0    ),  // ms
    E_L_     (-70.0    ),  // mV
...
nest::iaf_neuron_alpha_online::State_::State_()
  : 
  v_(-70.0),
    I_(0.0),  
    r_ (0)
...
nest::iaf_neuron_alpha_online::~iaf_neuron_alpha_online()
{
  ...
  std::cout<<"iaf_neuron_alpha_online destruct counter="<<iaf_neuron_alpha_online::id_gl<<"\n";
  iaf_neuron_alpha_online::id_gl--;
...
void nest::iaf_neuron_alpha_online::Parameters_::get(DictionaryDatum &d) const
{
...
void nest::iaf_neuron_alpha_online::Parameters_::set(const DictionaryDatum& d)
{
...
void nest::iaf_neuron_alpha_online::State_::get(DictionaryDatum &d, const Parameters_& p) const
{
  def<double>(d, names::V_m, v_); // Membrane potential
}

void nest::iaf_neuron_alpha_online::State_::set(const DictionaryDatum& d, const Parameters_& p)
{
  updateValue<double>(d, names::V_m, v_);
}


nest::iaf_neuron_alpha_online::Buffers_::Buffers_(iaf_neuron_alpha_online &n)
  : logger_(n)
{}

nest::iaf_neuron_alpha_online::Buffers_::Buffers_(const Buffers_ &, iaf_neuron_alpha_online &n)
  : logger_(n)
{}
    ...
    B_(*this)
{
  ...
  std::cout<<"non-copy iaf_neuron_alpha_online::id_gl "<<iaf_neuron_alpha_online::id_gl<<"\n"; 
  ...
  //nest::iaf_neuron_alpha_online::sh_gl=0;
}

nest::iaf_neuron_alpha_online::iaf_neuron_alpha_online(const iaf_neuron_alpha_online& n)
  : Archiving_Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
{
  ...
  std::stringstream r;
  iaf_neuron_alpha_online::id_gl++;
  id_neuron=iaf_neuron_alpha_online::id_gl;
  std::cout<<"copy iaf_neuron_alpha_online::id_gl "<<iaf_neuron_alpha_online::id_gl<<"\n";
  r << iaf_neuron_alpha_online::id_gl;
  //r<<++nest::iaf_neuron_alpha_online::sh_gl;
  //id_gl=nest::iaf_neuron_alpha_online::sh_gl;
  ...
  behav_learning_loc->read_learn_set_desired_from_file(s_directory_learn_set_desired);//perepisat vizov
   behav_learning_loc->read_learn_set_input_from_files(s_directory_learn_set_input,iaf_neuron_alpha_online::id_gl); //ano
  /*learn function*/
}

void nest::iaf_neuron_alpha_online::set_learning_variables(DictionaryDatum &d,double t,std::vector<double > &currents)//prepare before learning
{
...
void nest::iaf_neuron_alpha_online::init_state_(const Node& proto)
{
  const iaf_neuron_alpha_online& pr = downcast<iaf_neuron_alpha_online>(proto);
  S_ = pr.S_;
}



void nest::iaf_neuron_alpha_online::init_buffers_()
{
...
void nest::iaf_neuron_alpha_online::calibrate()
{
  B_.logger_.init();
...
double nest::iaf_neuron_alpha_online::normalization(double_t v)
{
...
bool nest::iaf_neuron_alpha_online::emission_procedure(double_t v)
{
  double Th=1.0;
  double k=0.1;
...
void nest::iaf_neuron_alpha_online::update(Time const & origin, const long_t from, const long_t to)
{
  //bool ok=true;
...
void nest::iaf_neuron_alpha_online::handle(SpikeEvent & e)
{
...
void nest::iaf_neuron_alpha_online::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::iaf_neuron_alpha_online::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}
