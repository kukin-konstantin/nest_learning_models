...
namespace nest
{
  namespace names
  {
    ...
    //learning modules
    //Supervised_behavior
    const Name trig_learn_oper("trig");
    const Name trig_seal("seal");
    const Name number_prim("number");
    const Name bias("bias");
    //Gradient_descent_behavior
    const Name time("time");
    const Name I_current("I_current");
    const Name labmda_function_number("labmda_function_number");
    const Name Th("Th");
    const Name k("k");
